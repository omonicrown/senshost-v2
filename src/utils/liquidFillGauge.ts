...
export function loadLiquidFillGauge(element: any, value: any, config: any) {
  var easing = [
    "easeElastic",
    "easeBounce",
    "easeLinear",
  ...
  var textRounder: any = function(value: any) {
    return Math.round(value);
  };
  if (parseFloat(textFinalValue) !== parseFloat(textRounder(textFinalValue))) {
    textRounder = function(value: any) {
  ...
  if (config.valueCountUp) {
    var textTween = function() {
      var i = d3.interpolate(textContent as any, textFinalValue);
      return function(t: any) {
        textContent = textRounder(i(t)) + percentText;
      };
    };
    text1
      .transition()
  ...
  function GaugeUpdater() {
    const update = (value: any) => {
      ...
      var textRounderUpdater: any = function(value: any) {
        return Math.round(value);
      };
      if (
        ...
        textRounderUpdater = function(value: any) {
          return parseFloat(value).toFixed(1);
        };
      }
      if (
      ...
      text1
        .transition()
        .duration(config.waveRiseTime)
        .tween("text", textTween).on("end", animateWave);
      text2
        .transition()
        .duration(config.waveRiseTime)
        .tween("text", textTween).on("end", animateWave);
      ...
      wave
        ...
        .duration(
          config.waveAnimate
            ? config.waveAnimateTime * (1 - Number(wave.attr("T")))
            : config.waveRiseTime
        )
        .ease(d3.easeLinear)
